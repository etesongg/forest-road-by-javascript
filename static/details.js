...
const getMntData = async() => {
    ...
    const mntiadd = Array.isArray(data.response.body.items.item)?data.response.body.items.item[0].mntiadd: data.response.body.items.item.mntiadd // 산 주소
...
const displayMntTrailInfo = async (trailArray) => {
    ...
    document.querySelector(".details-mnt-related-images > .row").innerHTML = MntTrailInfoHTML
}
